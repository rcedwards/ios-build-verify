...
- **Logical points, not pixels.** AXe (`axe tap`, `axe swipe`, `axe describe-ui`) consumes logical points. `xcrun simctl io <UDID> screenshot` writes a PNG in pixels at the device's native scale (3× on iPhone 17 / 17 Pro / 17 Plus). Mixing them sends taps far off-screen and looks like a silent failure. Divide pixel coordinates by the scale factor, or get logical-point frames directly from `axe describe-ui` (sample JSON walk in `prompts/agent-ui-audit-validation.md`).
- **Empty AXTree on launch = modal gating.** `launch_app.sh` exit 5 with `children: []` means a modal (review prompt, onboarding, system alert, IAP paywall, custom permission primer) is gating describe-ui before `FIRST_SCREEN_ID` can be polled. The full-tree dump is empty; reach the dismiss button via `axe describe-ui --point <x>,<y> --udid <UDID>`. See SKILL.md "Common first-real-app friction" item 6.
